...
## 8. Known gaps (recorded, not built)

Written down so they are not rediscovered. Neither is a notification problem, and neither should be fixed by adding one.

1. **Stored tier disagreeing with Stripe.** `users.subscription_tier` is written by the Stripe webhook (`invoice.paid`; `customer.subscription.updated` wrote it too until 2026-09-15). If an account's stored tier ever disagrees with its live Stripe subscription and no later event corrects it — a lost webhook, a manual edit, a plan changed in the Stripe dashboard with delivery failing — every cap decision uses the wrong limit. Concretely: `downgrade_grace_started` measures the overflow against the tier Stripe reports and will stay silent for a clock that looks false, while `apply_overflow_fallback` still locks against the stored tier when that clock expires 14 days later. Low probability. **The fix is a scheduled reconciliation** that compares stored tier and status against Stripe for every subscribed account and corrects drift — which needs the scheduled-job infrastructure this app does not have yet (every dormancy check today is lazy, run on page loads and `/admin`). Not a notification, and not something to patch per call site.

   *Added 2026-09-15.* The job has two constraints it must honour. **(a) Exempt accounts.** `web/src/lib/billing-exempt.ts` lists accounts whose plan was set by hand and have no Stripe subscription — today only `admin@moxieyachting.com` (`2255a040-7300-407c-bc31-7fb35b383d14`, set by `20260915_second_admin_identity.sql`). Stripe's answer for them is always "no plan"; the job must call `isTierReconciliationExempt` and skip them. The webhook and the grace notifier already do. **(b) Paid state, not price.** A subscription's item price is not evidence of payment — Stripe applies a price swap when it is requested. The webhook no longer writes the tier on `customer.subscription.updated`; `invoice.paid` writes the tier the latest paid invoice charged for (`web/src/lib/subscription-tier.ts`). Reconciliation must compare against that, not against `items.data[0].price`.

2. **Recovery from Stripe's `unpaid` state reads as a resubscription.** A restore is classified by whether `users.past_due_since` was still set (20261003): set means a failed payment recovered on the same subscription (`vessel_reactivated_payment_recovered`, emails); clear means a new subscription after a cancellation (`vessel_reactivated`, in-app). The webhook records Stripe's `unpaid` as `canceled` and clears `past_due_since`, so an `unpaid` subscription that is later paid is classified as a resubscription and notified in-app only. Stripe stops retrying `unpaid` subscriptions, so this recovery is normally the owner paying deliberately — the in-app treatment is usually right — but it is a misclassification by construction, not by judgement.
