...
16. ~~Full tier's billing model is per-vessel, not per-account — undecided whether that's actually intended.~~ **Resolved:** account-level, confirmed by decision. Subscription creation moved to a new standalone action (`dashboard/upgrade/actions.ts`'s `createFullAccessUpgradeIntent`), not scoped to any vessel and guarded against creating a second subscription for an account already on active Full — the two independent $149/yr subscriptions this item flagged live are exactly what that guard now prevents. The badge fee stays a separate, per-vessel, one-time `PaymentIntent` regardless of tier (real physical good, real per-unit cost — deliberately not absorbed into the subscription); `payment/actions.ts`'s `createBadgeFeeIntent` no longer takes a tier at all. A Basic vessel under a Full account is expected and fine: tier is purely an account-wide feature gate now, unrelated to which specific vessels exist. `users.stripe_subscription_id` (new column) tracks the one subscription per account directly, so no code ever needs to guess which vessel "owns" a subscription.
