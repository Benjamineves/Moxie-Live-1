...
**Activation logic — two explicit paths, same outcome.** An earlier version of this section described a single trigger (a paid `setup_fee` row) as if every signup produced one — written before the two-tier structure was final. It didn't hold up once Full's checkout copy promised the sticker "at no extra charge": Full has no separate setup-fee charge to record. Rather than invent a synthetic $0 `setup_fee` row just to keep one trigger uniform, `vessel_payments` stays an honest ledger of real money movement only, and activation is explicitly two paths:
```
Path A — Basic tier (one-time setup fee):
...
Path B — Full tier (annual subscription, first invoice):
  On the subscription's first invoice paid (Stripe webhook: invoice.paid,
  billing_reason='subscription_create'):
    → set vessels.qr_status = 'active' (same permanence guarantee as Path A)
    → set vessels.qr_generated_at = now()
    ...
    → no setup_fee vessel_payments row is created for this path — Full's
      price already includes activation; a 'subscription' row records the
      real amount charged instead

Renewal invoices (billing_reason='subscription_cycle') on an
already-active vessel:
  → record another vessel_payments row (payment_type='subscription') for
    billing history
  → no further effect on qr_status (already active) or subscription_tier
    (already 'full')
...
**Note on `vessel_payments.stripe_payment_intent_id`:** despite the name, this column holds two different kinds of Stripe object ID depending on `payment_type` — a PaymentIntent ID (`pi_...`) for Path A's one-time `setup_fee` rows, or an Invoice ID (`in_...`) for Path B's `subscription` rows. Both serve as the idempotency key against duplicate webhook deliveries (enforced by a unique index — see `20260827_payment_integration.sql`); Stripe removed the direct `Invoice.payment_intent` field in recent API versions, and reaching the real nested PaymentIntent from an invoice now needs its own expand-and-fetch, which isn't worth it purely for a dedup key. Documented on the column itself via `COMMENT ON COLUMN`, not just here.
...
3. ~~Hosting/database~~ — **Resolved, and pinned down.** Next.js + Supabase + Vercel confirmed. Two Supabase projects exist on the account; **`imhoeviehenluccjgnru` is the live one — full schema (`mxe_id`, `users`, `vessel_documents`, everything `moxie_digital_schema.sql` describes), and everything built so far (the payment/storage_type migration, RLS, the five MXE-0000{1-5} fixtures, the Wayfinder-duplicate cleanup) has landed there.** `web/.env.local`'s `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY` correctly point at it — confirmed, don't change. The other project is an older, minimal schema (`owners`/`vessel_docs` naming, no `mxe_id`) predating everything in this repo's `supabase/` folder; it's currently paused and not in use. (An earlier diagnostic pass in this build process briefly got the two projects backwards after a diagnostic query ran against the wrong browser tab — noted here so it doesn't resurface.)
...
11. **Generate real Supabase types.** All four Supabase client factories (`lib/supabase/{service,server,browser}.ts`, `lib/supabase-public.ts`) plus `lib/owner-verify.ts`'s standalone admin client currently type against `lib/supabase/schema-stub.ts` — a hand-written permissive stub (every table's Row/Insert/Update typed `any`) added to fix a real `tsc` failure: this SDK version collapses `.from(table)` types to `never` when no `Database` generic is supplied at all, so the stub was the minimum needed to get a clean type check while payment work was in flight. It restores compilation but gives zero column-level checking — exactly the kind of gap that let the `owners`/`vessel_docs` vs. `users`/`vessel_documents` schema mismatch go unnoticed for as long as it did (§9-A3). Run `supabase gen types typescript --project-id imhoeviehenluccjgnru > web/src/types/database.ts`, then swap `PermissiveDatabase` for the real generated `Database` type in all five files above. Expect a second wave of type errors when this happens — Supabase's generator typically types Postgres `numeric` columns as TS `string` (JS `number` can't represent arbitrary precision), which will conflict with code that currently assumes `number` for fields like `length_ft`/`draft_ft`/`amount_cents`. Deliberately deferred as its own pass rather than folded into the payment build — not blocking, but shouldn't be forgotten either.
12. ~~Debounce/cancel the checkout intent creation on tier toggle~~ — **Resolved.** Was creating a new abandoned Stripe PaymentIntent/Subscription on every tier click (confirmed in testing: six calls from ordinary toggling in one session; later, two more abandoned subscriptions from the same pattern, cluttering the event log mid-debugging). `payment/PaymentForm.tsx`'s effect now debounces `createIntentForTier` by 500ms and ignores stale responses via a `cancelled` flag — a plain debounce, not true cancellation, since a server action can't be aborted mid-flight the way a fetch can; the fix is to not call it until the tier selection settles, not to cancel a call already in progress.
13. **Ownership keys on email string, not the Supabase Auth ID — security item, tighten before real customers exist.** `dashboard/new/actions.ts`'s `createVessel` and `payment/actions.ts`'s `createIntentForTier` both resolve `owner_id` by looking up `public.users` by email and reusing that row's `id` if a match exists, rather than always using the signed-in Auth user's own `id`. A signup with an email matching an existing `public.users` row silently inherits that row's identity — and every vessel already attached to it — instead of getting a fresh account. This is deliberate (it's what let a real login "claim" the pre-seeded founder account without manual UUID reconciliation) and harmless today with zero real customers and only seed data in the table, but it's a real collision surface the moment genuine customer emails start populating it: any two signups that ever share an email string (re-signup after deletion, a typo, a pre-provisioned row, anything) would silently share one account.
    - **Email verification does not currently gate this in code.** `SignupForm.tsx`'s own post-signup copy ("If confirmation is required... If confirmations are disabled...") shows the application has no idea which applies — that's decided entirely by the Supabase project's Authentication → Providers → Email → "Confirm email" toggle, external to this codebase. Neither `createVessel` nor `createIntentForTier` checks `user.email_confirmed_at` anywhere. On the live project, confirmations are on right now (evidenced by an actual test signup requiring a confirmation-link click before intake proceeded), so an unconfirmed identity currently can't reach the matching code at all — `signUp()` withholds the session until the link is clicked, and there's no session to attempt a match with. But that protection is a side effect of an external, mutable dashboard setting, not something this code enforces — if that toggle is ever flipped, or this app is ever pointed at a differently-configured Supabase project, an unverified signup would immediately inherit whatever account and vessels already exist under that email string, with zero code change and no warning.
    - **Fix direction (not applied — flagged only):** key strictly on `auth.users.id` for both lookup and creation, dropping the email-fallback match entirely. If claiming the pre-seeded founder account by email is still wanted, do it once, deliberately (by hand, or a one-time claim script) — not as a standing mechanism every signup passes through. Separately, add an explicit `email_confirmed_at` check in code wherever identity is trusted, so this isn't resting entirely on a dashboard toggle staying where it currently is.
14. **Payment page copy assumes first-time activation.** `payment/page.tsx` now also serves active-Basic vessels upgrading to Full (Account & Billing panel's "Upgrade to Full Access" CTA), but the page's header copy ("Your sticker prints and ships the moment this clears... anyone who scans it from then on sees a real, active vessel identity") and the `Activate {vesselName}` button label were written for a not-yet-active vessel and read oddly on that upgrade path, where the vessel is already active and has a sticker. Accepted as-is for now — Basic is hidden from the tier picker on this path so the checkout itself isn't confusing, just the surrounding copy. Worth tightening with upgrade-aware copy before real launch.
15. **Pre-existing lint violation in `PaymentForm.tsx`'s debounce logic.** Lines 75-107 (the `useEffect` that debounces `createIntentForTier` on tier toggle, item 12 above) call `setClientSecret(null)`/`setError(null)` synchronously inside the effect body, which trips `react-hooks`'s `set-state-in-effect` rule (`eslint` reports it as an error, not a warning). Flagged during the Account & Billing panel round while linting unrelated changes in the same file — pre-existing, not introduced then, and not addressed since: fixing it risks touching already-tested checkout behavior for a lint-only violation, so it was left alone rather than folded into an unrelated change. Still open.
16. **Full tier's billing model is per-vessel, not per-account — undecided whether that's actually intended.** Every vessel goes through its own independent Basic/Full choice and payment regardless of the owner's existing subscription state (`dashboard/new/actions.ts`'s `createVessel` never reads `subscription_tier`; `payment/actions.ts`'s `createIntentForTier` only special-cases an already-*active* vessel, never an owner who's already Full elsewhere). Confirmed live, not theoretical: one test account picked "Full" for two separate vessels and ended up with two independent $149/yr Stripe subscription objects on the same customer, not one plan covering both — `subscriptions.create()` is called unconditionally with no check for an existing subscription on that customer. The open product question this exposes: is Full meant to be an account-wide plan that should cover every vessel an owner registers once purchased, or is it legitimately a per-vessel add-on that happens to reuse the same price and label? The code today implements the latter by accident, not by decision, and it silently stacks billing rather than erroring or asking. Also unresolved as part of the same question: if Full is meant to be account-wide, should a Full owner's new vessel skip the tier picker/payment step and auto-activate, and does a Basic vessel under an otherwise-Full account even make sense? Flagged, not built — this deserves the same weight as a real pricing/packaging decision, not a quick default.
17. **Registration/title document required at initial signup — discussed and deliberately deferred, not forgotten.** Came up directly while scoping the locked-field correction-request flow (item requiring a registration/title upload for post-signup *corrections* to HIN/make/model/year/length/draft/engine — see the vessel edit sections above): the same logic would seem to argue for requiring that document at signup too, as a platform-wide trust and fact-checking signal on every vessel, not just ones being corrected later. Deliberately not doing that here. Two separate reasons, both real: (1) it conflicts directly with the low-friction self-serve signup goal the product is built around — Diane's original feedback was specifically that boaters are casual about documentation, and a chunk of the product's reason for existing is meeting people where that's true rather than gatekeeping on it, the same tension already resolved once for email+password over magic link (item 7); (2) a stored file that nothing checks isn't a real trust signal, just a folder — meaningfully verifying it needs document parsing/extraction against the typed fields, which is real scope on its own, not a one-line addition to intake. This is a packaging/trust-model decision on the same order as item 16's billing question, not a quick add-on — revisit deliberately, as its own decision, when it's actually being weighed.
18. **Full-tier email reminders (insurance/registration expiry) — intentionally not built, and not just a technical backlog item.** Field-level scope for this was already decided: insurance is the real/primary reminder, registration expiry is real/secondary, boater card and photos are out since neither expires. Not building it anyway, because it's tangled with a bigger open question rather than blocked on anything technical: item 16 (is Full a per-account plan or a per-vessel add-on?) already establishes that Full's shape is unsettled, and reminders were specified early — back when §9-B8 split Basic's in-app-only indicators from Full's added email layer — as *the* concrete thing that differentiates paying for Full at all, without that differentiator having been revisited since. Building the email-sending mechanics now would be building the one specified piece of a value proposition that hasn't been rethought, on top of a billing model that's already known to be wrong in practice (item 16's confirmed double-subscription case). Revisit alongside item 16's billing-model decision, as one decision, not as a standalone feature to pick up separately.
...
## 11a. Field-visibility governance — keeping §11's patent guarantee true as the schema grows

Claim 1 depends on one thing staying true forever: a vessel's fields only ever leave the server through a role-aware filter, never as a raw row. This section is the audit of whether that's actually the case today, plus what it would take to have code — not developer memory — enforce it going forward.

### Audit: every path that reads `vessels` and what it returns

| Path | Filtering | Notes |
|---|---|---|
| `[mxeId]/page.tsx` (public + owner render) | `filterVesselForRole()` | ✅ Correct |
| `api/vessels/:mxeId?role=` | `filterVesselForRole()` | ✅ Correct. marina/coastguard branches 401 before querying further |
| `api/vessels/:mxeId/preview` | `toPreview()` | ✅ Correct, but a **second, separate** allow-list function — see below |
| `dashboard/[mxeId]/qr/page.tsx` | Narrow `.select("mxe_id, vessel_name, owner_id, qr_status")` | Not through the shared filter, but the query itself only names 4 columns — functionally equivalent, just ad hoc rather than centralized |
| `dashboard/[mxeId]/payment/{page,actions}.tsx`, `payment/processing/page.tsx` | Narrow `.select(...)` per query, few columns each | Same as above — ad hoc, not centralized, but nothing beyond what the public tier would show anyway |
| `api/stripe/webhook/route.ts` | Narrow `.select("id, qr_status")`, server-only, never returned to a client | N/A — not a data-out path |
| `dashboard/page.tsx` (owner's own vessel list) | **`select("*")`, no filter** | ⚠️ Flagged below |
| `admin/stickers/page.tsx` | **`select(...)` naming owner_name/owner_email across every customer, no filter** | ⚠️ Deliberate exception, flagged below |

**`dashboard/page.tsx` — not a current leak, but a real gap.** It fetches the complete row (`select("*")`) for every vessel the signed-in user owns, then renders a card server-side with only a handful of fields. Today nothing leaks: the render happens entirely inside the async server component with no client component receiving the raw `vessel` object as a prop, so the unfetched fields (HIN, insurance policy numbers, emergency contact) never reach the browser. But the query doesn't go through `filterVesselForRole(vessel, "owner")`, so if this page is ever refactored to pass `vessel` into a client component (an easy, natural-looking change — e.g. to add interactivity to a card), the full unfiltered row would start shipping to the browser with nothing in the code flagging that as a regression. Worth changing to go through the shared filter, or at minimum to a narrow explicit `.select(...)`, next time this file is touched — not urgent since there's no live exposure, but it's the one place a future edit could introduce one silently.

**`admin/stickers/page.tsx` is a deliberate, now-documented exception.** It intentionally returns `owner_name`/`owner_email` across every active customer at once — that's the entire point of a fulfillment queue. It bypasses `filterVesselForRole` on purpose, gated instead by `requireAdmin()` (role='admin' AND an `ADMIN_EMAILS` allowlist — see `lib/admin-verify.ts`) rather than the owner/public role system. Recorded here, and in a comment on the file itself, specifically so this reads as a conscious call rather than something a future audit rediscovers with alarm.

**`toPreview()` is a second allow-list, independent of `filterVesselForRole()`.** Both are already "default-deny" in *structure* — each builds a new object by explicitly naming which fields to include, rather than starting from the full row and omitting some. A new column added to `vessels` doesn't appear in either output unless a developer explicitly writes `newField: v.newField` into one of them. That part already works. The gap is that **nothing requires a developer to make that decision at all** — a new field just silently doesn't show up anywhere (safe, if invisible) until someone adds it to whichever tier(s) it belongs in, and there's no prompt forcing that to happen at the time the column is added, and no single source of truth listing "every field, and which roles see it" that both functions are checked against.

### The rule, going forward

**Any new column added to `vessels` requires an explicit role-visibility decision — which roles see it, if any — made at the same time as the migration that adds it, not discovered later.** In practice: when adding a column, also decide and write down (in `filterVesselForRole`, `toPreview` if relevant, and this doc) which of public/owner/marina/coastguard get it. Absence from both functions is the correct default for anything not yet decided — just make the decision on purpose rather than by accident.

### What actual code-level enforcement would take (not built — reporting only, per instruction)

Three options, increasing in robustness and in effort:

1. **Cheapest — a lint/CI check, not a code change.** Forbid `.select("*")` (or any wildcard/broad select) against `vessels` outside `lib/vessel-service.ts`. A custom ESLint rule or even a plain `grep` in CI catches the actual failure mode seen in `dashboard/page.tsx` — a query pulling more than a route needs — without touching the filter functions themselves. Doesn't stop someone from explicitly naming every column, but that's a much more deliberate act than typing `*`.
2. **Medium — a runtime schema per role, replacing the hand-written object literals.** Define a Zod (or similar) schema per role tier and have `filterVesselForRole` parse the row through it instead of constructing the object by hand. This gives an actual runtime guarantee — any field not in the schema is stripped even if it somehow made it into the input — and is a natural evolution of the current allow-list pattern rather than a rewrite. Still requires a developer to add a field to the right schema, but a missing decision now fails safe (field dropped) rather than just being invisible.
3. **Heaviest — compile-time enforcement via branded types.** Make the "unfiltered" `VesselRecord` type structurally incompatible with whatever a `NextResponse.json()` call or a public-facing component's props are typed to accept, so only the output of `filterVesselForRole`/`toPreview` satisfies that type. This turns "someone returned a raw row from a new route" into a TypeScript compile error instead of a code-review miss. Most robust option here, and the only one that would have caught `dashboard/page.tsx`'s `select("*")` even in its current (currently-harmless) form — but real implementation effort (module boundaries, likely a lint rule to catch `as`-casts used to bypass the brand) for a team this size, and probably not worth it until there are more hands touching this code than there are today.

One limitation worth naming regardless of which of these (if any) gets built: the RLS policy added on `vessels` (build spec §9-A3's migration) is **row-level only** — it controls which rows the anon key can read (`is_public = true`), not which columns come back within an allowed row. It doesn't provide column-level protection on its own; a future public-facing route that ran `getPublicSupabase().from("vessels").select("*")` against an allowed row would still get every column back. RLS is a backstop against the wrong *rows*, not a substitute for the role filter on *fields*.
...
| **Stripe webhook receiver** | Payment + subscription events | Must verify Stripe's signature before trusting any payload. Handles both activation paths from §4: Basic's `setup_fee` paid → `qr_status='active'`; Full's first invoice paid → `qr_status='active'` + `subscription_tier='full'`. Renewal invoices just log payment history. Subscription `past_due`/`canceled` after dunning → `subscription_tier='basic'` **without** touching `qr_status` |
