...
| [`moxie_digital_marina_access_spec.md`](moxie_digital_marina_access_spec.md) | **Built 2026-09-18.** The customer-facing marina role view: owner-granted by join code, per-marina, fixed field set with documents at the owner's option. `/marina` roster, `/marina/join`, `/admin/marinas`, `npm run marina-poster` | None known |
...
| 2 | **Transfer buyer with no subscription** | Decided 2026-09-15: a 30-day window to choose a plan, enforced by the scheduler (not built; scheduler spec §3.2). Until then a buyer who receives a vessel by transfer keeps it active without subscribing, indefinitely |
| 3 | **Stored tier disagreeing with Stripe** | Needs a scheduled reconciliation job. The app has **no scheduled jobs**: every dormancy check runs lazily on page loads and `/admin`. Recorded in the dormant identity spec §8, with two constraints for that job: skip the hand-set accounts in `lib/billing-exempt.ts` (the admin), and compare against the paid invoice, not the subscription's price |
