      ...
      <div className={dormant.isDormant ? "pointer-events-none opacity-60" : undefined} aria-disabled={dormant.isDormant}>
        ...
        {missingRegExpiry || missingInsExpiry ? (
          <ExpiryDateNudge
            ...
            href={`/dashboard/${encodeURIComponent(tier.mxe_id)}/documents#documents`}
          />
        ) : null}
      ...
      <section className="mx-auto max-w-lg px-5 pb-10 md:px-8">
        ...
        */}
        ...
        <dl className="mt-4 rounded-xl border border-[var(--divider)] bg-[var(--white)] p-5 shadow-sm">
          ...
          <Row label="Public notes" value={tier.public_notes} />
        </dl>
        ...
        {/*
          Vessel actions — the three requests that change what this record
          IS, rather than what it says: a locked-field correction, a
          decommission, an ownership transfer. They were previously loose
          triggers stacked with the ordinary field editors, where a
          transfer of ownership read as no more consequential than editing
          a slip number. Set apart here, and deliberately last.

          Still inside the dormant lock above, not moved outside it: a
          dormant vessel can't be edited, and that has to include these.
        */}
        <div className="mt-16 rounded-xl border border-[var(--divider)] bg-[var(--gray-bg)] p-6">
          <h2 className="font-[family-name:var(--font-display)] text-2xl font-light text-[var(--navy)]">
            Vessel actions
          </h2>
          <p className="mt-1.5 font-[family-name:var(--font-dm)] text-xs text-[var(--text3)]">
            Changes to the record itself. Each one is reviewed before it takes effect.
          </p>

          {/* [&_button]:text-left overrides the UA stylesheet's centred
              button text, which is only visible now that these triggers
              wrap inside a container instead of sitting alone in a
              right-aligned column. Colour, size, weight and underline —
              the actual trigger styling — are untouched. */}
          <div className="mt-5 flex flex-col items-start gap-4 border-t border-[var(--divider)] pt-5 [&_button]:text-left">
          ...
          {/*
            Presentational shell only — TransferOwnershipPanel itself is
            untouched and unaware it's in here. Wrapping rather than
            restyling keeps the panel usable anywhere else it might be
            mounted, and keeps this card's chrome out of a component whose
          ...
          */}
          {!isDecommissioned && !needsActivation ? (
            <div className="mt-5 rounded-xl border border-[var(--gold-line)] bg-[var(--navy-deep)] p-5">
              ...
              <p className="mt-1.5 font-[family-name:var(--font-dm)] text-xs text-[rgba(255,255,255,.55)]">
                Hand this vessel — its record, documents and history — to a new owner.
              </p>
              <div className="mt-4 flex flex-col items-start gap-3 border-t border-[rgba(255,255,255,.12)] pt-4 [&_button]:text-left">
