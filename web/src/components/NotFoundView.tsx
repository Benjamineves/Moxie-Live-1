...
/**
 * The shell every 404 in the app renders through, so the four boundaries
 * differ only in their words.
 *
 * Why there are four: `app/not-found.tsx` catches every unmatched URL in
 ...
 * is a real vessel reached by a route that does not exist. That cost real
 ...
 * a badge or a batch where the thing that failed genuinely was one.
 */
export function NotFoundView({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[var(--cream)] px-6 text-center">
      ...
      <Link className="mt-8 font-[family-name:var(--font-dm)] text-sm text-[var(--blue-fg)] underline" href={href}>
        {action}
      </Link>
    </div>
  );
